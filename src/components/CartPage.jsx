...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (auth.currentUser != null) {
        dispatch(SetAuthenticationEmail(auth.currentUser.email));
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        const updatedItems = currentItems.map((item) =>
          item.cartId === cartId
            ? { ...item, quantity: value.valueAsNumber }
            : item
        );
  ...
  const ConfirmOrder = async () => {
    ...
    const UserCart = doc(db, "Users", AuthEmail);
    const OrderItems = Cart.map(({ name, quantity, price, Addons, img1 }) => ({
      name,
      quantity,
      price,
      Addons,
    ...
    const TotalBill = charges.reduce(
  ...
  if (LoginStatus == true) {
    ...
    if (Cart.length > 0) {
      ...
      return (
        ...
        >
                ...
                <div className="CartEachitemSpace" key={item.name}>
                  ...
                  <div className="CartAddonsContainer">
                    {item.Addons.map((addon) => {
                      return (
                        <p className="CartAddonNames" key={addon.name}>
                          &nbsp;&nbsp; | {addon.name}{" "}
                        </p>
                      );
                    })}
          ...
          <div
            className="cartTotalSummary"
            style={{ height: (window.innerHeight * 10) / 100 }}
            onClick={() => {
              setMenuModal("flex");
            }}
          >
          ...
          {/* Modal */}
          <div
            className="AddToCartMenuModal"
            style={{ display: MenuModal, height: window.innerHeight }}
          >
            <div className="MenuModalWholeCard">
              <div className="MenuModalItemPart">
                <div className="CartProceedtoBuyBox">
                  <p className="Captions" style={{ color: "#FFF" }}>
                    Confirm Order !
                  </p>
                  <p className="CatagoryCaptions" style={{ color: "#FFF" }}>
                    Please pay ₹{" "}
                    {(
                      Number(ItemTotal) +
                      (Number(ItemTotal) * 9) / 100 +
                      (Number(ItemTotal) * 9) / 100 +
                      20
                    ).toFixed(2)}{" "}
                    at your Doorstep!
      ...
      return (
        <div className="NotLoginCart" style={{ height: window.innerHeight }}>
          <img
            src={`${process.env.PUBLIC_URL}/photos/LoginEmptyCart.webp`}
            width={"35%"}
            color="white"
          />
          <p className="NotLoginText">OOps Nothing in your Cart</p>
          <Button className="NotLoginCartButton" onClick={() => navigate("/")}>
            <p className="NotLoginCartButtonText"> Continue Shopping</p>
          </Button>
        </div>
      );
    }
    ...
    return (
      <div className="NotLoginCart" style={{ height: window.innerHeight }}>
        <img
          src={`${process.env.PUBLIC_URL}/photos/CartNothing.svg`}
          width={"35%"}
          color="white"
        />
        <p className="NotLoginText">OOps. You have to login first</p>
        <Button
          className="NotLoginCartButton"
          onClick={() => navigate("/LoginPage")}
        >
          <p className="NotLoginCartButtonText"> Login</p>
        </Button>
      </div>
    );
  }
